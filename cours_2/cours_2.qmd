...
## Construire un dashboard : la colonne vertébrale {style="font-size:0.85em"}

::::: columns
::: {.column width="40%"}
1.  **Sources** : CRM, web analytics, campagnes média...
2.  **Préparation** : scripts R/Python (nettoyage, agrégations, calculs KPI).
3.  **Modèle métier** : définitions partagées (qu'est-ce qu'un lead, un CPA, une conversion ?).
4.  **App / BI** : Shiny, Looker Studio, Tableau, Power BI, etc.
5.  **Diffusion** : accès sécurisé, rafraîchissements, alertes, rituels d'usage.
:::

::: {.column width="60%"}
![](images/clipboard-4071259443.png)
:::
:::::

## Organisation d'une app (exemple Shiny) {style="font-size:0.85em"}

1.  **Cadres de décision** : regrouper les KPI par question (Que se passe-t-il ? Pourquoi ? Que faire ?).
2.  **UI** : définir la structure (`fluidPage`, `tabsetPanel`, `valueBox`) pour guider l'œil.
3.  **Server** : créer des blocs *reactive* qui transforment les inputs en KPI, graphiques, alertes.
4.  **Storyline** : chaque onglet raconte un morceau de l'histoire (diagnostic → recommandation).

```{r}
#| eval: false
ui <- fluidPage(
  titlePanel("Pilotage acquisition"),
  fluidRow(
    valueBoxOutput("cpa"),
    valueBoxOutput("conv_rate"),
    valueBoxOutput("budget")
  ),
  tabsetPanel(
    tabPanel("Vue globale", plotOutput("cpa_plot")),
    tabPanel("Segments", plotOutput("segment_plot"))
  )
)
```

## Panorama express des outils {style="font-size:0.85em"}

| Outil | Quand l'utiliser ? | Points clés |
|:-----------------------|:-----------------------|:-----------------------|
| **Shiny** | Besoin de logique R, personnalisation fine | Contrôle total, nécessite hébergement (Shiny Server/Posit Connect). |
| **Looker Studio** | Partage rapide avec marketing/clients | No-code, connecteurs GA4/BigQuery, gouvernance limitée. |
| **Power BI / Tableau** | Organisations équipées BI/IT | Gestion des droits, rafraîchissements planifiés, collaboration. |
| **Quarto / Notebooks** | Livrables mixtes analyse + slides | Reproductible, parfait pour documenter la méthodologie. |

## Plan de démo pour l'app Shiny

1.  **Lancement** : ouvrir `app_shiny.R`, montrer les `valueBox` et filtres en action.
2.  **Scénario utilisateur** : filtrer sur Facebook vs AdWords, observer l'impact sur CPA/taux de conversion.
3.  **Bouclage avec la data** : rappeler le pipeline (sources → `pivot_longer()` → KPI → dashboard) et les décisions possibles.
